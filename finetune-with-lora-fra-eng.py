...
During training this script evaluates on the dev split and logs:
- eval_loss
- eval_wer
- eval_cer

Best checkpoint selection is based on eval_wer.
...
def load_mucs_entries(extract_dir: Path) -> List[Dict[str, Any]]:
    ...
    for utt_id, transcript in utt2text.items():
        ...
        entries.append({
            ...
            "end_time": float(end or (start + dur)),
            "utt_id": utt_id,
        })
...
def download_switchlingua_audio(
    sl_dir: Path,
    lang_cfg: Dict[str, str],
    skip_download: bool = False,
) -> str:
    ...
    have_audio = (
        audio_csv.exists()
        and audio_csv.stat().st_size > 0
        and audio_files_dir.exists()
        and any(audio_files_dir.iterdir())
    )
    ...
    if have_audio:
        print(
            f"  Local audio data found for {lang_cfg['expected_lang']}. "
...
def load_switchlingua_entries(audio_snapshot: str, lang_cfg: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    if audio_col is None or text_col is None:
        raise RuntimeError(
            ...
            f"  Need at least one of: file_name/filename/audio_file/path (got {audio_col})\n"
            f"  And: text/transcript/sentence (got {text_col})\n"
            f"  Available columns: {list(df.columns)}"
        )

    print(
        f"  -> audio: {audio_col!r}, text: {text_col!r}, "
    ...
    if first_lang_col or second_lang_col:
        first_langs = (
            set(df[first_lang_col].astype(str).str.strip().unique())
            if first_lang_col else set()
        )
        second_langs = (
            set(df[second_lang_col].astype(str).str.strip().unique())
        ...
        if contaminants:
            ...
            print(
                f"  Filtered to {expected_lang}-containing rows: {before} -> {len(df)} "
        ...
        print(
            f"  WARNING: no language columns found. Cannot verify content matches "
            f"{expected_lang}. Proceeding anyway."
        )

    if conv_type_col is not None:
        ct = (
            df[conv_type_col].astype(str).str.lower().str.strip()
        ...
        print(
            f"  conversation_type: single_turn={single_turn_count}, "
    ...
    print(
        f"  Skipped: {missing_audio} missing audio, {bad_text} bad text, "
        f"{bad_audio} unreadable audio"
    )
    print(f"  Loaded {len(entries)} entries ({sum(e['duration'] for e in entries) / 3600:.2f}h)")
    return entries
...
class GenericAudioDataset(Dataset):
    def __init__(self, entries: List[Dict[str, Any]], processor: Wav2Vec2Processor):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        dur = (float(end) - start) if end is not None else None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "LoRA fine-tuning on Nh of code-switching data. "
    ...
    print(f"\n[2] Splitting train ({args.train_hours}h) + dev (~5min) + test_pool (rest)...")
    ...
    print(
        f"  Test pool: {len(test_entries):>5} entries, "
    ...
    if is_meta_checkpoint:
        proc_path = (
            os.path.join(args.model_path, "processor")
            if os.path.isdir(os.path.join(args.model_path, "processor"))
            else args.model_path
        )
        model_load_path = (
            os.path.join(args.model_path, "meta_model")
        ...
        if args.vocab_json and os.path.exists(args.vocab_json):
            ...
            vocab = build_joint_vocab([clean_text(e["text"]) for e in train_entries + dev_entries])
    ...
    sanity_batch = collator([train_dataset[0], train_dataset[min(1, len(train_dataset) - 1)]])
    print(f"  Batch keys sanity check: {list(sanity_batch.keys())}")
    ...
    print("  Eval set during training = dev_entries")
    trainer.train()

    print(f"\n[6] Saving final adapter to {args.output_dir}...")
    ...
    meta = {
        ...
        "best_metric_eval_wer": best_metric,
        "eval_set_is": "dev_entries",
        "manifests": {
    ...
    print(f"  Best eval WER:   {best_metric}")
...
#     # Optional sanity check: confirm batch keys look right for Wav2Vec2ForCTC.
#     sanity_batch = collator([train_dataset[0], train_dataset[min(1, len(train_dataset) - 1)]])
#     print(f"  Batch keys sanity check: {list(sanity_batch.keys())}")
...
#         eval_strategy="steps",
#         eval_steps=args.eval_steps,
#         save_strategy="no",              # important: avoid PEFT checkpoint save bug here
#         load_best_model_at_end=False,    # requires checkpoint saves, so keep False here
#         remove_unused_columns=False,
#         dataloader_num_workers=0,
#         push_to_hub=False,
#         report_to="none",
